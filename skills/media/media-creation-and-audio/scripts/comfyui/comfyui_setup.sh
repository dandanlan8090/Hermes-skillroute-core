...
if curl -fsS "http://<ip>:$PORT/system_stats" >/dev/null 2>&1; then
    log "Server already running on port $PORT — skipping launch."
    log "Stop with \`$COMFY_BIN stop\` if you want a fresh start."
    curl -fsS "http://<ip>:$PORT/system_stats" | python3 -m json.tool 2>/dev/null || true
    log "Done."
    exit 0
fi
...
log "  Web UI: http://<ip>:$PORT (open in browser)"

...
|---|---|---|
| Base URL | `http://<ip>` | `https://cloud.comfy.org` |
...
```bash
curl -s "http://<ip>/history"          # all
curl -s "http://<ip>/history/{id}"     # one prompt_id
```
...
```bash
# Local (direct bytes)
curl -s "http://<ip>/view?filename=ComfyUI_00001_.png&subfolder=&type=output" \
  -o output.png
...
```bash
# Local
wscat -c "ws://<ip>/ws?clientId=MY-UUID"
...
```bash
# Image
curl -X POST "http://<ip>/upload/image" \
  -F "image=@photo.png" -F "type=input" -F "overwrite=true"
# Returns: {"name": "photo.png", "subfolder": "", "type": "input"}

# Mask (linked to a previously uploaded image)
curl -X POST "http://<ip>/upload/mask" \
...
curl -s "http://<ip>/object_info" | python3 -m json.tool

# Specific node
curl -s "http://<ip>/object_info/KSampler"

# Models per folder (local)
curl -s "http://<ip>/models/checkpoints"
curl -s "http://<ip>/models/loras"
...
```bash
# View queue
curl -s "http://<ip>/queue"

# Clear all pending
curl -X POST "http://<ip>/queue" \
  -H "Content-Type: application/json" \
  -d '{"clear": true}'
...
curl -s "http://<ip>/system_stats"

# Free GPU memory
curl -X POST "http://<ip>/free" \
  -H "Content-Type: application/json" \
...
curl -X POST "http://<ip>/manager/queue/install" \
  -H "Content-Type: application/json" \
  -d '{"git_url": "https://github.com/user/comfyui-node.git"}'

# Check install queue status
curl -s "http://<ip>/manager/queue/status"

# Install model
curl -X POST "http://<ip>/manager/queue/install_model" \
  -H "Content-Type: application/json" \
